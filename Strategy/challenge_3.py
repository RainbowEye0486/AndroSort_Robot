...
PK_bit = False
penalty_offense = False
penalty_defense = False
goal_kick = False
pk_last_cmd = 'r'
...
def strategy():
    ...
    """
    global ball, robots, enemies, goal_kick, pk_last_cmd
    ...
    if goal_kick:
        ...
        cmd[1] = 'N'
    if penalty_offense:
        if robots[1].role == Role.ASSISTER:
            cmd[1] = 'N'
        else:
            cmd[0] = 'N'
    if penalty_defense:
...
def execute_job(id):
    ...
    if robo.job == Job.MOVE:
        ...
        if robo.target[0] != -1:
            check_boundary_ball(robo)
            force = 'big'
            kickable_dist = 4.5 * CM_TO_PIX  # the distance between arrival and the robot should be
            kickable_ang = 15 / 180 * math.pi  # acceptable angle error when kicking
...
def move(robo, arrival, ways=['', '', '', ''], accurate=True):
    ...
    for i in range(1, 11):
        if dist >= motion['BOUND'][0] * CM_TO_PIX and count != MAX_STEPS:
            too_close, position = is_close_ball(position, direction, robo.MOTION['MOVE'][move_way]['BOUND'][0] * CM_TO_PIX)
            if (not too_close) or move_way == 'BACK':
...
def find_aim_point(x, y, goal):
    ...
    if sizes:
        ...
        comp = -1.1
        for i in range(len(sizes)):
            if size_max == 0 or dist_max == 0:
                return [ball.pos[0] + SIDE * 3 * CM_TO_PIX, ball.pos[1]], 0.2
            else:
