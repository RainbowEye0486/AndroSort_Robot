...
def thread_enemy():
    ...
    if mask & (mask_col == 'e'):
        cv2.imshow("enemy_mask", enemy_mask)
...
def thread_color1():
    ...
    if mask & (mask_col == '1'):
        cv2.imshow("color1_mask", color1_mask)
...
def thread_color2():
    ...
    if mask & (mask_col == '2'):
        cv2.imshow("color2_mask", color2_mask)
...
def thread_color3():
    ...
    if mask & (mask_col == '3'):
        cv2.imshow("color3_mask", color3_mask)
        ...
        for our in our_pos:
            for color2 in color2_pos:
                if get_distance(our, color2) < 30:
                    ...
                    our_dir[1] = [(color2[0] - our[0]) / dist, (color2[1] - our[1]) / dist]
                    if error_open:
                        q = error_correct(our)
                        cv2.circle(show, (q[0], q[1]), 3, (252, 255, 255), -1)
                        our_data[1] = [q[0], q[1]]
        ...
        for our in our_pos:
            for color3 in color3_pos:
                if get_distance(our, color3) < 30:
                    ...
                    our_dir[2] = [(color3[0] - our[0]) / dist, (color3[1] - our[1]) / dist]
                    if error_open:
                        q = error_correct(our)
                        cv2.circle(show, (q[0], q[1]), 3, (252, 255, 255), -1)
                        our_data[2] = [q[0], q[1]]
